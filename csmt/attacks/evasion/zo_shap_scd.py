...
np.random.seed(10)
class ZOShapSCDMethod(AbstractEvasion):

    def gradient_estimation(self,mu,q,x,kappa,target_label,const):
        grad_est=0
        d=x.shape[1]
        # idx_coords_random=np.random.randint(d,size=q)
        # import_index=self.feature_importance.argsort()[-2:][::-1]
        import_index=np.array([0])
        idx_coords_random=np.random.choice(import_index,size=q,replace=True)
        # print(self.feature_importance)
        # print(idx_coords_random)
