    ...
    def __init__(self, name, data_dir=None, mode='origin', verbose=True):
        ...
        if name in self.COGDL_GRAPH_CLASSIFICATION_DATASETS:
            ...
            if mode == 'origin':
                ...
                self.index_test=np.where(self.test_mask.cpu().numpy()==True)[0]
                self.index_train=np.where(self.train_mask.cpu().numpy()==True)[0]
                self.index_val=np.where(self.val_mask.cpu().numpy()==True)[0]
