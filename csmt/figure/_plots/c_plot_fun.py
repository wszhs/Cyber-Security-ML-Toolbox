...
"""
from csmt.figure._plots import CPlot
from csmt.figure._plots.plot_utils import create_points_grid
# from secml.array import CArray
from csmt.figure.type_utils import is_list
import numpy as np
...
class CPlotFunction(CPlot):
    ...
    def plot_fun(self, func, multipoint=False,
        ...
        clip_min = -np.inf if vmin is None else vmin
        clip_max = np.inf if vmax is None else vmax
        ...
        if plot_background is True:
            ...
            if colorbar is True:
                some_y=np.linspace(grid_points_val_reshaped.min(),grid_points_val_reshaped.max(), num=20, endpoint=True)
