...
def create_points_grid(grid_limits, n_grid_points):
    ...
    x_grid_points =np.linspace(x_min - padding_x, x_max + padding_x, num=n_grid_points, endpoint=True)
    y_grid_points = np.linspace(y_min - padding_y, y_max + padding_y, num=n_grid_points, endpoint=True)
    # Create the grid
    pad_xgrid,pad_ygrid=np.meshgrid(x_grid_points,y_grid_points,indexing='xy')

    pad_grid_point_features=np.ma.concatenate((pad_xgrid.reshape(pad_xgrid.size, 1),pad_ygrid.reshape(pad_ygrid.size, 1)),axis=1)
