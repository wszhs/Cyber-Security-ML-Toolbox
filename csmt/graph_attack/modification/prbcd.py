...
from csmt.graph_attack.base import ModificationAttack, EarlyStop
from csmt.classifiers.graph.evaluator import metric
from csmt.classifiers.graph.utils import utils
from csmt.classifiers.graph.utils.normalize import SPARSEAdjNorm
...
class PRBCD(ModificationAttack):
    ...
    def modification(self,
        ...
        """
        features_attack = utils.feat_preprocess(features=features_origin,
                                                feat_norm=model.feat_norm if feat_norm is None else feat_norm,
                                                device=self.device)
        ...
        adj = utils.adj_to_tensor(adj).to(self.device).coalesce()
        ...
        def evaluate(features, adj):
            pred = model(features, adj_norm_func(adj))
