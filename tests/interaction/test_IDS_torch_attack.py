...
LastEditTime: 2021-07-03 11:43:45
...
from tests.interaction.interaction_loss import (InteractionLoss, get_features,
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc_1 = nn.Linear(in_features=4*6*8, out_features=30)
...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(in_channels=1, out_channels=4, kernel_size=2, stride=1)
        self.fc_1 = nn.Linear(in_features= 4* 7 * 9, out_features=100)
...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.fc_1=nn.Linear(in_features=80,out_features=64)
...
def PGD(X, y,model):
    ...
    for i in range(num_steps):
        ...
        if lam > 0:  # Interaction-reduced attack
            only_add_one_perturbation, leave_one_out_perturbation = sample_for_interaction(delta, sample_grid_num,grid_scale, image_width,sample_times)
...
X_train=X_train.reshape(-1,1,8,10)
X_test=X_test.reshape(-1,1,8,10)
model = Net()
model2 = Net()
...
count2=0
for i in range(100):
    adv_X=PGD(X_test[i:i+1],y_test[i:i+1],model)
    print(count1)
    adv_y=predict(X_test[i:i+1],y_test[i:i+1],model)
    model1_y=predict(adv_X[99:100],y_test[i:i+1],model)
    model2_y=predict(adv_X[99:100],y_test[i:i+1],model2)
    if adv_y!=model1_y:
        count1+=1
    if adv_y!=model2_y:
        count2+=1
