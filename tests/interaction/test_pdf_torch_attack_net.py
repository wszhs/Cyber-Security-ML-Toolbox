...
LastEditTime: 2021-07-23 11:41:57
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc_1 = nn.Linear(in_features=10*7*1, out_features=30)
...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(in_channels=1, out_channels=4, kernel_size=5, stride=1)
        self.fc_1 = nn.Linear(in_features= 4* 11 * 5, out_features=100)
...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.fc_1=nn.Linear(in_features=135,out_features=32)
...
def PGD(X, y,model,lam):
    ...
    for i in range(num_steps):
        ...
        if lam > 0:  # Interaction-reduced attack
            ...
            #     pairwise_arr[0,con_i]=average_pairwise_interaction_i
            # average_pairwise_interaction=pairwise_arr.mean()*50
...
#     X=X_test[i:i+1]
#     y=y_test[i:i+1]
#     adv_X,deltas=PGD(X,y,model,lam=1)
#     delta=deltas[9:10]
#     average_pairwise_interaction=get_average_pairwise_interaction(model,X,y,delta,sample_grid_num=sample_grid_num,grid_scale=grid_scale,times=times)
...
#     # average_pairwise_interaction1=pairwise_arr.mean()*50
...
for i in range(100):
    adv_X,deltas=PGD(X_test[i:i+1],y_test[i:i+1],model,lam=10)
